// src/app/book/BookingClientPage.tsx
"use client";
...
export default function BookingClientPage() {
  ...
  useEffect(() => {
    async function fetchRestaurants() {
      ...
      setRestaurants(data);
      if (!searchParams.get('restaurantId') && data.length > 0) {
  ...
  useEffect(() => {
    ...
    async function fetchMenus() {
      ...
      const data = await res.json();
      setMenus(data);
      if (!searchParams.get('menuId') && data.length > 0) {
        setSelectedMenuId(data[0].id.toString());
      }
    }
    fetchMenus();
  }, [selectedRestaurantId, searchParams]);
